 Vue.component("addmanager", {
	...
	template: `
	<div class="signup_form_div" style="overflow:auto;height:100vh;">
        <form class="signup_form">
            <label class="signup_label_header">Register Manager:</label>
            <br>
            ...
            <br><input type="submit" class="nav_button login_button" v-on:click="addManager" value="Add manager"/>
	...
	methods : {
		...
		addManager:function(){
			...
			this.invalidPassword = (this.user.password === null) || (this.user.password.length < 6);
				...
				axios.post('rest/users/',this.user)
					.then(function (response) { 
							if(response.data){
								...
								var u=response.data;
								return axios.post('rest/baskets/createBasket/'+u.username).then(response=>(router.go(-1)))	
							}
							else{
